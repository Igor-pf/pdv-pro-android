...
class MainActivity : AppCompatActivity() {
    ...
    override fun onNewIntent(intent: Intent?) {
        ...
        intent?.let {
            if (it.scheme == "kioskpdv" && it.data?.host == "payment_result") {
                ...
                // Variáveis detalhadas da Stone SmartPOS para gravar log/NSU
                val nsu = it.getStringExtra("nsu") ?: it.getStringExtra("AcquirerTransactionKey") ?: ""
                val authCode = it.getStringExtra("authorization_code") ?: ""
                val brand = it.getStringExtra("brand") ?: ""
                
                // Envia o callback para o Javascript processar a baixa da mesa/comanda
                val js = "javascript:if(window.onStonePaymentResult) { window.onStonePaymentResult('$status', '$orderId', '$amount', '$nsu', '$authCode', '$brand'); }"
                webView.evaluateJavascript(js, null)
                
                if (status == "APPROVED" || status == "SUCESSO") {
                    Toast.makeText(this, "Pagamento Aprovado na Maquininha!", Toast.LENGTH_SHORT).show()
                } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // [MDM] Tentar entrar em Kiosk Mode (Screen Pinning / Lock Task)
        try {
            startLockTask()
        } catch (e: Exception) {
            // Pode falhar em emuladores antigos ou se não for ativado pelo gestor
        }
    ...
    private fun setupWebView(url: String) {
        ...
        // [MDM] Persistência de Sessão e Cookies (Cloudflare & Entra ID)
        ...
        webView.addJavascriptInterface(AndroidPayment(this), "AndroidPayment")
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                // [MDM] Forçar gravação dos cookies de sessão (Entra ID) no disco
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
